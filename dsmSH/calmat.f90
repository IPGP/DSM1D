...
subroutine calmatc( nlayer,vnp,vra,con,rpow,w1dn,w2dn,ra,m,work )
  ...
  ! parameter check
  if ( rpow .gt. maxrpow ) stop 'Invalid arguments.(calmatc)'
...
! (n-1) degrees polynomial a(n) and (m-1) degrees polynomial b(n).
  ...
  if (n+m-1.ne.l) stop 'Invalid arguments.(pmulti)'
  do i=1,l
     c(i)=0.d0
  enddo
...
! which is the product of (n-1) degrees polynomial 'p(n)' and 'con'.
  ...
  if ( n.gt.maxn ) stop 'Degrees of a polynomial is too large.(pinteg)'
  if ( snp.ge.vnp ) snp = 1
...
subroutine polint( n,p,x1,x2,pint )
  ...
  if ( n.gt.maxn) stop 'Degrees of a polynomial is too large.(polint)'
...
subroutine calg2( l,m,spo,r0,mt,mu0,coef,ga,a,ga2,dr,g2,ig2,iwork)
  ...
  if (  ((m.eq.-2).or.(m.eq.-l)).and.(ig2.eq.0) ) then
     if (iwork==100.and.l<10) write(*,*) "in/dclisb0"
     call dclisb0( ga2,3,1,2,cg2,eps,dr,z,ier)
     ig2= ig2+1
  else
     if (iwork==100.and.l<10) write(*,*) "in/dscbsub0"
...
subroutine calg_force(l,m,spo,r0,f,mu0,coef,ga,a,ga2,dr,g2,ig2,iwork)
  implicit none
  real(kind(0d0)), parameter :: pi=3.1415926535897932d0
  integer :: l,m,iwork
  real(kind(0d0)) :: spo,r0,f(3),mu0
  complex(kind(0d0)) :: coef,ga(8),a(4),ga2(2,3),g2(*)
  integer :: i,itmp,ig2
  real(kind(0d0)) :: b1,sgn,ier
  complex(kind(0d0)) :: cg2(3),dr(3),z(3)
  real(kind(0d0)) :: eps
  data eps / -1.d0 /
  ...
  if (iabs(m)==1) then
    b1 = dsqrt(dble(2*l+1)/(16.d0*pi))
    cg2(2) = dcmplx(b1)*dcmplx(sgn*f(3),f(2))
  endif
