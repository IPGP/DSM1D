...
!-----------------------------------------------------------------------
  ...
  if (myrank==0) write(*,'("--> DSM : input broadcast")')
  call MPI_BARRIER(MPI_COMM_WORLD,ierr)
  ...
  if (myrank==0) write(*,'("--> DSM : compute grid points")')
  ...
  if (myrank==0) write(*,'("--> DSM : compute matrix elements")')
  ...
  if (myrank==0) write(*,'("--> DSM : compute dislocation")')
  nn = nnlayer + 1
  do ir0 = 1, r0_n
...
open(2,file=coutfile,status='unknown',form='unformatted',access='direct',recl=3*6*nsta*2*kind(0e0))

  do iwork = imax-myrank,imin,-mysize !!! worker loop
    i = iwork
    write(*,'("send work ",i5," to proc ",i3," at ",e12.5)') iwork,myrank,dble(iwork)/tlen
     ...
     write(2,rec=i+1) displacementsngl(1:3,1:6,1:nsta)
