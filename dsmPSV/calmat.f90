...
subroutine cala0(nlayer,omega,omegai,t,h1x,h1y,h1z,h2L,h2N,h3ax,h3ay,h3az,h4aL,h4aN, &
  h5ax,h5ay,h5az,h6aL,h6aN,h7x,h7y,h7z,h8L,h8N,coef1,coef2,a0)
...
subroutine calg( l,m,coef1,coef2,lsq,ecC0,ecF0,ecL0,ya,yb,yc,yd,ra,r0,mt,g )
  ...
  if ( iabs(m).eq.0 ) then
     dd = dcmplx( 2.d0 * b1 * mt(1,1) / ( r0 * r0 ) )  / ( dcmplx( ecC0 - 4.d0 / 3.d0 * ecL0 ) &
       * coef2   + dcmplx( 4.d0/3.d0*ecL0 ) * coef1 )
     ee = dcmplx( 0.d0 )
  endif
...
subroutine calparderiv(omegar,omegai,l,c,dcdr,z,dzdr,ra, par0,par1,par2)
  ...
  tmp4 = l2 * c(1) * z(1) + ra2 * dcdr(2) * dzdr(2) - ra * ( dcdr(2) * z(2) + c(2) * dzdr(2) ) + &
    c(2) * z(2) - lsq * ( - ra * ( c(1) * dzdr(2) + dcdr(2) * z(1) )   + ( c(1) * z(2) + c(2) * z(1) ) )
